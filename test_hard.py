...
def test_hard(data, t,
         model = keras.models.load_model('Xception DNN Traffic Sign Model Fine Tuned.h5'),
         class_names = ['Unknown','Stop','Yield','Red Light','Green Light','Roundabout','Right Turn Only',
    ...
    #==================== Softmax threshold=========================================
    probs = np.max(y_prob,1) # highest softmax probability
    fail_idx = probs < sm_threshold # idx of datapoints in the unknown category
    ...
    correct_idx = t == y
    correct_prob = np.max(y_prob[np.bitwise_and(correct_idx,~fail_idx)], 1)
...
#     print('Standard deviation of confidence of correct guess: ', std_correct_prob)
    ...
    incorrect_idx = t != y
    incorrect_prob = np.max(y_prob[np.bitwise_or(incorrect_idx,fail_idx)], 1)
